...
const Listbox = /*#__PURE__*/_react.default.forwardRef((props, ref) => {
  ...
  const filteredItems = groupable ? data.filter(item => {
    ...
    const groupValue = (0, _get.default)(item, groupBy, '') ||
    // FIXME-Doma
    ...
    item.parent?.[_getDataGroupBy.KEY_GROUP_TITLE];
  ...
  const renderItem = ({
    ...
    const active = activeItemValues?.some(v => (0, _utils.shallowEqual)(v, value));
  ...
  (0, _hooks.useMount)(() => {
    const itemIndex = (0, _findIndex.default)(filteredItems, item => item[valueKey] === activeItemValues?.[0]);
