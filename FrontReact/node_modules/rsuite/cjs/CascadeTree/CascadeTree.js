...
const CascadeTree = (0, _utils.forwardRef)((props, ref) => {
  ...
  const parentMap = (0, _react.useMemo)(() => (0, _utils2.getParentMap)(data, item => childrenMap.get(item) ?? item[childrenKey]), [childrenMap, childrenKey, data]);

  // Flatten the tree data
  const flattenedData = (0, _react.useMemo)(() => (0, _utils3.flattenTree)(data, item => childrenMap.get(item) ?? item[childrenKey]), [childrenMap, childrenKey, data]);
  ...
  const onSearchCallback = (0, _react.useCallback)((value, _items, event) => onSearch?.(value, event), [onSearch]);
  const {
    items,
    searchKeyword,
    setSearchKeyword,
