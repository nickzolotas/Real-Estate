...
const MultiCascader = (0, _utils3.forwardRef)((props, ref) => {
  ...
  } = (0, _Picker.useFocusItemValue)(selectedPaths?.[selectedPaths.length - 1]?.[valueKey], {
    rtl,
    data: flattenData,
    valueKey,
  ...
  const handleMenuPressEnter = (0, _hooks.useEventCallback)(event => {
    ...
    const checkbox = overlay.current?.querySelector(`[data-key="${focusItemValue}"] [type="checkbox"]`);
    if (checkbox) {
   ...
   */
  let hasValue = selectedItems.length > 0 || Number(valueProp?.length) > 0 && (0, _isFunction.default)(renderValue);
  if (hasValue && (0, _isFunction.default)(renderValue)) {
    selectedElement = renderValue(value.length ? value : valueProp ?? [], selectedItems, selectedElement);
