...
const CheckPicker = (0, _utils2.forwardRef)((props, ref) => {
  ...
  } = (0, _Picker.useFocusItemValue)(value?.[0], {
    data,
    valueKey,
  ...
  const handleMenuPressEnter = event => {
    ...
    const focusItem = data.find(item => (0, _utils2.shallowEqual)(item?.[valueKey], focusItemValue));
    setValue(nextValue);
  ...
  const selectedItems = data.filter(item => value?.some(val => (0, _utils2.shallowEqual)(item[valueKey], val))) || [];

  /**
   * 1.Have a value and the value is valid.
   * 2.Regardless of whether the value is valid, as long as renderValue is set, it is judged to have a value.
   */
  let hasValue = selectedItems.length > 0 || value?.length > 0 && (0, _isFunction.default)(renderValue);
  const {
    prefix,
    merge
  ...
  if (value?.length > 0 && (0, _isFunction.default)(renderValue)) {
